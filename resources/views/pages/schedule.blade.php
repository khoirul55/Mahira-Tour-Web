    ...
    <div class="container-main">
        <div class="flex gap-2 justify-center flex-wrap">
            <button @click="activeFilter = 'all'; document.querySelectorAll('[data-schedule-card]').forEach(el => el.style.display = '')"
                    ...
                    :class="activeFilter === 'all' ? 'border-[#001D5F] bg-[#001D5F] text-white shadow-[0_8px_25px_rgba(0,29,95,0.25)]' : 'border-[#E8EBF3] bg-white text-gray-500'">
                Semua Jadwal
            </button>
            ...
            <button @click="activeFilter = '{{ $route }}';
                    ...
                    class="px-8 py-3 rounded-full font-semibold text-[15px] cursor-pointer transition-all duration-300 border-2"
                    :class="activeFilter === '{{ $route }}' ? 'border-[#001D5F] bg-[#001D5F] text-white shadow-[0_8px_25px_rgba(0,29,95,0.25)]' : 'border-[#E8EBF3] bg-white text-gray-500'">
                {{ $route }}
            </button>
            @endforeach
        </div>
...
<section class="py-20 bg-white">
    <div class="container-main">
                ...
                <div class="rounded-2xl overflow-hidden h-full flex flex-col transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl group bg-white shadow-[0_4px_15px_rgba(0,29,95,0.08)]"
                    ...
                    <a href="{{ route('schedule.detail', ['id' => $schedule['id'], 'slug' => \Illuminate\Support\Str::slug($schedule['package_name'])]) }}" 
                       class="block relative h-[350px] overflow-hidden cursor-pointer no-underline bg-gray-100">
                        ...
                        {{-- Status Badge --}}
                        <span class="absolute top-3 right-3 z-10 flex items-center gap-1.5 px-3.5 py-1.5 rounded-full font-bold text-xs bg-gradient-to-br from-[#D4AF37] to-[#F4D03F] text-[#001D5F] shadow-[0_4px_12px_rgba(212,175,55,0.4)]">
                    ...
                    <div class="p-5 flex-1 flex flex-col gap-3.5 relative z-[5]">
                        <h3 class="text-xl font-bold leading-snug m-0 text-[#001D5F]">{{ $schedule['package_name'] }}</h3>
                        
                        <div class="flex flex-wrap gap-2">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-[#E8EBF3] text-[#001D5F]">
                            ...
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white bg-[#D4AF37]">
                        ...
                        <div class="flex gap-2 mt-auto">
                            ...
                            <a href="{{ route('register', ['schedule_id' => $schedule['id']]) }}" 
                               class="flex-1 inline-flex items-center justify-center gap-2.5 py-4 rounded-lg text-[15px] font-semibold no-underline transition-all duration-300 hover:-translate-y-0.5 bg-[#001D5F] text-white border-2 border-[#001D5F] hover:bg-[#D4AF37] hover:border-[#D4AF37] hover:shadow-lg hover:shadow-[#D4AF37]/30">
                            ...
                            <a href="{{ route('schedule.detail', ['id' => $schedule['id'], 'slug' => \Illuminate\Support\Str::slug($schedule['package_name'])]) }}" 
                               class="flex-1 inline-flex items-center justify-center gap-2.5 py-4 rounded-lg text-[15px] font-semibold no-underline transition-all duration-300 hover:-translate-y-0.5 bg-white text-[#001D5F] border-2 border-[#001D5F] hover:bg-[#D4AF37] hover:border-[#D4AF37] hover:text-white hover:shadow-lg hover:shadow-[#D4AF37]/30">
        ...
        <div class="text-center py-20">
            ...
            <h4 class="text-xl font-bold mb-2 text-[#001D5F]">Belum Ada Jadwal Tersedia</h4>

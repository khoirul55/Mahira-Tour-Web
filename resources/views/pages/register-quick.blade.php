...
<section class="min-h-screen flex items-center justify-center relative overflow-hidden py-16 md:py-8 bg-gradient-to-br from-[#001D5F] to-[#002B8F]">
    ...
    <div class="max-w-[600px] w-full mx-auto relative z-[1] px-4">
        ...
        <div class="bg-white rounded-3xl md:rounded-2xl p-12 md:p-6 relative z-[2] shadow-[0_20px_60px_rgba(0,0,0,0.3)]">
            ...
            {{-- Package Summary --}}
            @if($selectedSchedule)
            <div class="rounded-2xl p-5 mb-8 bg-gradient-to-br from-[#F8F9FF] to-[#E8EBF3] border-2 border-[#D4AF37]">
                <h3 class="text-xl font-bold mb-4 text-[#001D5F]">{{ $selectedSchedule['package_name'] }}</h3>
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-[#001D5F]">
                        <svg class="w-5 h-5 shrink-0 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        <span>{{ date('d M Y', strtotime($selectedSchedule['departure_date'])) }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-[#001D5F]">
                        <svg class="w-5 h-5 shrink-0 text-[#D4AF37]" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"/></svg>
                        <span>{{ $selectedSchedule['departure_route'] }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-[#001D5F]">
                        <svg class="w-5 h-5 shrink-0 text-[#D4AF37]" fill="currentColor" viewBox="0 0 24 24"><path d="M22 16.21v-1.895l-1.5-1.5v-7.396c0-.854-.552-1.609-1.368-1.873L12.66 1.356c-.427-.139-.89-.139-1.317 0L4.868 3.546c-.816.264-1.368 1.02-1.368 1.873v7.396l-1.5 1.5v1.895h2v1.79h16v-1.79h2z"/></svg>
                        <span>{{ $selectedSchedule['airline'] }}</span>
                    </div>
                </div>
                <div class="text-[1.8rem] md:text-[1.4rem] font-extrabold mt-4 text-[#D4AF37]">
            ...
            <form action="{{ route('register.submit') }}" method="POST" id="quickBookingForm" class="space-y-6">
                ...
                @else
                    <div>
                        <label class="block font-bold text-[15px] mb-2 text-[#001D5F]">
                            Pilih Paket Keberangkatan <span class="text-red-500">*</span>
                        </label>
                        <select name="schedule_id" required
                                class="w-full px-5 py-3.5 rounded-xl text-base outline-none transition-all duration-300 cursor-pointer border-2 border-[#E8EBF3] font-[inherit] focus:border-[#001D5F] focus:ring-3 focus:ring-[#001D5F]/10">
                ...
                <div>
                    ...
                    <input type="text" name="full_name" required minlength="3"
                           ...
                           value="{{ old('full_name') }}"
                           class="w-full px-5 py-3.5 rounded-xl text-base outline-none transition-all duration-300 border-2 border-[#E8EBF3] font-[inherit] focus:border-[#001D5F] focus:ring-3 focus:ring-[#001D5F]/10">
                </div>
                ...
                {{-- Catatan --}}
                <div>
                    <label class="block font-bold text-[15px] mb-2 text-[#001D5F]">Catatan Tambahan (Opsional)</label>
                    <textarea name="notes" rows="3"
                              placeholder="Tuliskan permintaan khusus jika ada..."
                              class="w-full px-5 py-3.5 rounded-xl text-base outline-none transition-all duration-300 resize-y border-2 border-[#E8EBF3] font-[inherit] focus:border-[#001D5F] focus:ring-3 focus:ring-[#001D5F]/10">{{ old('notes') }}</textarea>
                </div>
            ...
            <div class="flex gap-4 p-4 rounded-xl mt-6 bg-amber-50 border-2 border-amber-500">
                ...
                <div class="flex-1">
                    <strong class="block mb-1 text-sm text-[#001D5F]">Langkah Selanjutnya:</strong>

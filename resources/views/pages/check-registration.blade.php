...
<section class="min-h-screen flex items-center justify-center relative overflow-hidden py-16 md:py-8 bg-gradient-to-br from-[#001D5F] to-[#002B8F]">
    ...
    <div class="max-w-[500px] w-full mx-auto relative z-[2] px-4"
        ...
        <div class="bg-white rounded-3xl md:rounded-2xl p-12 md:p-6 mb-8 md:mb-6 shadow-[0_20px_60px_rgba(0,0,0,0.3)]">
            ...
            <h1 class="text-[2rem] md:text-[1.5rem] font-extrabold mb-2 flex items-center gap-3 text-[#001D5F]">
                ...
                <div class="grid grid-cols-2 md:grid-cols-1 gap-3 mb-6">
                    <label class="cursor-pointer">
                        ...
                        <div class="flex flex-col md:flex-row items-center justify-center md:justify-start gap-2 p-4 md:p-3 rounded-xl text-center md:text-left transition-all duration-300
                             ...
                             :class="loginMethod === 'phone' ? 'border-2 border-[#001D5F]' : 'border-2 border-[#E8EBF3] hover:bg-gray-50'">
                            <svg class="w-6 h-6 md:w-5 md:h-5" :class="loginMethod === 'phone' ? 'text-[#001D5F]' : 'text-gray-400'" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 2C6.477 2 2 6.477 2 12c0 1.89.525 3.66 1.438 5.168L2 22l4.832-1.438A9.955 9.955 0 0012 22c5.523 0 10-4.477 10-10S17.523 2 12 2zm0 18a8 8 0 01-4.257-1.222l-.293-.175-3.045.799.813-2.971-.192-.304A7.963 7.963 0 014 12a8 8 0 1116 0 8 8 0 01-8 8z"/></svg>
                            <span class="font-semibold text-sm" :class="loginMethod === 'phone' ? 'text-[#001D5F]' : 'text-gray-500'">Nomor WhatsApp</span>
                        </div>
                    </label>
                    <label class="cursor-pointer">
                        ...
                        <div class="flex flex-col md:flex-row items-center justify-center md:justify-start gap-2 p-4 md:p-3 rounded-xl text-center md:text-left transition-all duration-300
                                    peer-checked:bg-[rgba(0,29,95,0.05)]"
                             :class="loginMethod === 'reg' ? 'border-2 border-[#001D5F]' : 'border-2 border-[#E8EBF3] hover:bg-gray-50'">
                            <svg class="w-6 h-6 md:w-5 md:h-5" :class="loginMethod === 'reg' ? 'text-[#001D5F]' : 'text-gray-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            <span class="font-semibold text-sm" :class="loginMethod === 'reg' ? 'text-[#001D5F]' : 'text-gray-500'">No. Registrasi</span>
                        </div>
                    </label>
                </div>
            ...
            <div class="text-center mt-6 text-sm text-gray-500">
                ...
                <a href="{{ route('register') }}" class="font-bold no-underline hover:underline text-[#001D5F]">Daftar Sekarang</a>
            </div>
        </div>
    </div>
</section>
